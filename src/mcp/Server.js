...
export class Server extends EventEmitter {
  ...
  async executeToolHandler(toolName, input) {
    switch (toolName) {
      case 'reason': return await this.handleReasoning(input);
      case 'memory-query': return await this.handleMemoryQuery(input);
      case 'execute-tool': return await this.handleToolExecution(input);
  ...
  async registerBuiltInTools() {
    this.exposedTools.set('reason', {
      ...
      outputSchema: {
        type: 'object',
        properties: {
          conclusions: { type: 'array', items: { type: 'string' }, description: 'Conclusions derived from reasoning' },
          confidence: { type: 'number', description: 'Confidence level of conclusions' },
          derivationSteps: { type: 'array', items: { type: 'string' }, description: 'Steps taken during the reasoning process' }
        }
      }
    });
    ...
    this.exposedTools.set('memory-query', {
      ...
      outputSchema: {
        type: 'object',
        properties: {
          results: { type: 'array', items: { type: 'object', properties: { id: { type: 'string' }, content: { type: 'string' }, confidence: { type: 'number' }, timestamp: { type: 'string' } } } },
