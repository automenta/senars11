...
export class TaskEditorComponent extends BaseComponent {
    ...
    _getKeyboardShortcuts() {
        return {
            ...
            'E': () => this.editTask(this.element.selected), // Shift+E for edit
            'd': () => this.deleteTask(this.element.selected),
            'D': () => this.deleteTask(this.element.selected), // Shift+D for delete
            'delete': () => this.deleteTask(this.element.selected),
            'p': () => this.adjustPriority(this.element.selected),
            'P': () => this.adjustPriority(this.element.selected), // Shift+P for priority
            'g': () => this._showGroupingMenu(),
            'G': () => this._cycleGroupingModes(),
            'C-g': () => this.cycleGroupingCriteria(), // Ctrl+G for cycling grouping
            'C-t': () => this.setGroupingCriteria('time-daily'), // Ctrl+T for time grouping
            'C-r': () => this.setGroupingCriteria('relationships'), // Ctrl+R for relationship grouping
            'C-s': () => this.setGroupingCriteria('similarity-content'), // Ctrl+S for similarity grouping
            'C-p': () => this.setGroupingCriteria('priority'), // Ctrl+P for priority grouping
            'C-e': () => this.expandAllGroups(),
            'C-h': () => this.collapseAllGroups(),
            'C-a': () => this._selectAllTasks(), // Select all tasks
            'C-u': () => this._clearSelection(), // Clear selection
            'C-d': () => this._duplicateTask(this.element.selected), // Duplicate task
            's': () => this._toggleSelection(this.element.selected), // Single selection
            'S': () => this._toggleSelection(this.element.selected), // Shift+S for selection
            'u': () => this._clearSelection(), // Unselect
            'up': () => this.element.up(),
            'down': () => this.element.down(),
            'pageup': () => this.element.pageup(),
            'pagedown': () => this.element.pagedown(),
            'home': () => this.element.setScroll(0), // Jump to top
            'end': () => this.element.setScrollPerc(100), // Jump to bottom
            'f': () => this._showFilterMenu(), // Filter tasks
            'F': () => this._showSearchDialog(), // Search tasks
            'x': () => this._executeSelectedTask(), // Execute selected task
            'X': () => this._executeAllSelectedTasks(), // Execute all selected tasks
            'c': () => this._copyTaskContent(this.element.selected), // Copy task content
            'C': () => this._copyTaskContent(this.element.selected), // Shift+C for copy
            'v': () => this._pasteTaskContent(), // Paste task content
            'V': () => this._pasteTaskContent() // Shift+V for paste
        };
    }
    ...
    _duplicateTask(index) {
        if (index >= 0 && index < this.tasks.length) {
            ...
            this.components.logViewer.addInfo(`📋 Duplicated task: ${originalTask.content}`);
        }
    }
    ...
    _executeSelectedTask() {
        ...
        if (selected.length > 0) {
            ...
            this.components.logViewer.addInfo(`🏃 Executing task: ${task.content}`);
        }
    }
    ...
    _executeAllSelectedTasks() {
        ...
        if (selected.length > 0) {
            ...
            this.components.logViewer.addInfo(`🏃 Executing ${selected.length} tasks...`);
        }
    }
    ...
    _copyTaskContent(index) {
        if (index >= 0 && index < this.tasks.length) {
            ...
            this.components.logViewer.addInfo(`📋 Copied task content to buffer: ${content}`);
    ...
    _pasteTaskContent() {
        ...
        this.components.logViewer.addInfo(`📋 Paste functionality - would paste task content here`);
    ...
    _applyPriorityFilter() {
        ...
        this.components.logViewer.addInfo('🔍 Priority filter menu would open here');
    }

    _applyStatusFilter() {
        this.emit('filter-status-requested');
        this.components.logViewer.addInfo('🔍 Status filter menu would open here');
    }

    _applyTimeFilter() {
        this.emit('filter-time-requested');
        this.components.logViewer.addInfo('🔍 Time filter menu would open here');
    }

    _showAllTasks() {
        this.emit('show-all-tasks-requested');
        this.components.logViewer.addInfo('📋 Showing all tasks');
    }

    _showSearchDialog() {
        this.emit('search-requested');
        this.components.logViewer.addInfo('🔍 Search dialog would open here');
    }
                ...
                onConfirm: () => {
                    ...
                    this.addVisualFeedback(null, { 
                        ...
                        callback: () => {
                            ...
                            this.components.logViewer?.addInfo(`🗑️ Task deleted: ${task.content.substring(0, 50)}${task.content.length > 50 ? '...' : ''}`);
                        }
                    });
                }
            });
                ...
                onConfirm: (newContent) => {
                    ...
                    this.addVisualFeedback(null, { 
                        ...
                        callback: () => {
                            ...
                            this.components.logViewer?.addInfo(`✏️ Task edited: ${oldTask.content.substring(0, 30)}... → ${newContent.substring(0, 30)}...`);
                        }
                    });
                }
            });
