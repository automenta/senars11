...
export const StatusBar = ({ status }) => {
  ...
  return React.createElement(
    ...
    React.createElement(
      Box,
      { flexDirection: 'row' },
      React.createElement(Text, { color: 'white', bold: true }, `${runningIndicator} ${connectionIndicator} ${connectionText} | `),
      ...
      React.createElement(Text, { color: alerts > 0 ? 'red' : 'white', bold: alerts > 0 }, `${alertIndicator} Alerts: ${alerts}`)
    ),
    React.createElement(
      Box,
      { flexDirection: 'row' },
