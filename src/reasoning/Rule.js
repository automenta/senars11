...
const resolveContext = (memoryOrContext, termFactory) => {
...
const updateMetrics = (metrics, success, time) => Metrics.update(metrics, success, time);
...
export class Rule {
    ...
    _updateIfChanged(prop, val) {
        ...
        const newConfig = prop === '_enabled' ? {...this._config, enabled: val} : null;
        return this._clone(prop === '_enabled' ? {} : {[prop]: val}, newConfig);
    }

    canApply(task) { return this._enabled && this._matches(task); }

    async apply(task, memoryOrContext, termFactory) {
        const { effectiveContext, effectiveMemory, effectiveTermFactory } = resolveContext(memoryOrContext, termFactory);
    ...
    _updateMetrics(success, time) {
        const metrics = updateMetrics(this._metrics, success, time);
