...
export function registerArithmeticOps(registry) {
    ...
    const unaryBinaryOp = (unaryFn, binaryFn, checkZero = false) => (...args) => {
        const nums = OperationHelpers.requireNums(args);
    ...
    registry.register('&-', unaryBinaryOp(x => -x, (a, b) => a - b));
    registry.register('&/', unaryBinaryOp(x => 1 / x, (a, b) => a / b, true));
}