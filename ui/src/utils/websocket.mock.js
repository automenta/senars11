...
class MockWebSocketService {
  ...
  connect() {
    // In mock, we're immediately connected
  ...
  async handleMessage(event) {
    try {
      ...
      // In the mock, we don't validate messages, just route them
      this.routeMessage(data);
    } catch (error) {
      console.error('Error parsing mock WebSocket message:', error);
    }
  ...
  sendMessage(message) {
    ...
    if (message.type === 'startDemo') {
      ...
      // Send some activity to make the demo look active
      setTimeout(() => {
        this.routeMessage({
          type: 'demoState',
          payload: { demoId: message.payload.id, status: 'running', progress: 25, currentStep: 'Processing input' }
        });
      }, 300);
      setTimeout(() => {
        this.routeMessage({
          type: 'demoState',
          payload: { demoId: message.payload.id, status: 'running', progress: 50, currentStep: 'Running inference' }
        });
      }, 600);
      setTimeout(() => {
        this.routeMessage({
          type: 'demoState',
          payload: { demoId: message.payload.id, status: 'running', progress: 75, currentStep: 'Generating output' }
        });
      }, 900);
      setTimeout(() => {
        this.routeMessage({
          type: 'demoState',
          payload: { demoId: message.payload.id, status: 'completed', progress: 100, currentStep: 'Completed' }
        });
      }, 1200);
    } else if (message.type === 'narseseInput') {
      // Simulate narsese input processing
