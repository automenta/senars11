...
class WebSocketService {
    ...
    async routeMessage(data) {
        ...
        if (type === 'connection_info') {
            ...
            // Handle basic connection data
            if (data.data) {
                // Process connection info as needed
                console.debug('WebSocket connection established with server', data.data);
            }
            return;
        }

        const commandHandlers = {
            demoControl: () => this.isTestEnvironment && this.handleDemoControl({type, payload}),
    ...
    handleDemoControl({payload: {command, demoId}}) {
        ...
        const generateRandomContent = () =>
            `<${getRandomElement(contentElements.subjects)} --> ${getRandomElement(contentElements.predicates)}>${getRandomElement(contentElements.punctuation)}`;
