...
import { Card, FeatureCard, StatCard, Button, StatusBadge, LoadingSpinner, EmptyState, ErrorState, GenericInputField, ToggleSwitch, TimeDisplay, KeyValueDisplay } from './components/shared/index.js';
...
const TestApp = () => {
  ...
  return React.createElement('div', { style: { padding: themeUtils.get('SPACING.LG') } },
    ...
    // Test cards
    React.createElement('div', { style: { display: 'flex', gap: themeUtils.get('SPACING.MD'), marginBottom: themeUtils.get('SPACING.LG') } },
    ...
    React.createElement('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: themeUtils.get('SPACING.MD'), marginBottom: themeUtils.get('SPACING.LG') } },
      React.createElement(Card, { title: 'Control Panel' },
        React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: themeUtils.get('SPACING.SM') } },
          React.createElement(Button, { onClick: handleIncrement }, `Count: ${count}`),
          React.createElement(StatusBadge, { status: count % 2 === 0 ? 'success' : 'warning', label: count % 2 === 0 ? 'Even' : 'Odd' }),
    ...
    React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: themeUtils.get('SPACING.MD') } },
      count < 3 ? 
