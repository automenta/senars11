/**
 * Enhanced WebSocket Service with proper connection lifecycle management
 * Provides resilient WebSocket communication following the pattern defined in PLAN.ui.md
 *
 * Key improvements:
...
const MESSAGE_HANDLERS = Object.freeze({
    demoControl: (wsService, data) => wsService.isTestEnvironment && wsService.handleDemoControl({
        type: data.type,
...
class WebSocketService {
    constructor(url, options = {}) {
        ...
        // Debounced message processor to prevent excessive UI updates
        this.debouncedRouteMessage = debounce(this.routeMessage.bind(this), 50);
    }
    ...
    disconnect() {
        ...
        this.pendingRequests.forEach((rejectFn) => {
            rejectFn(new Error('WebSocket disconnected'));
        });
    ...
    _generateFallbackData() {
        ...
        this.fallbackDataStore.beliefs = [
            {
                id: 'fallback_belief_1',
                term: '<fallback --> belief>.',
                priority: 0.8,
                creationTime: now,
                type: 'belief',
    ...
    /**
     * Send message with request-response pattern support
    ...
    sendMessage(message, timeout = 5000) {
        if (!message) {
            ...
            return Promise.reject(new Error('Cannot send null/undefined message'));
        }
        ...
        if (this.state === ConnectionState.CONNECTED && this.ws?.readyState === WebSocket.OPEN) {
            // If the message has a request ID or needs to expect a response
            if (message.expectResponse) {
                ...
                // Regular message without expecting response
                try {
                    const serializedMessage = JSON.stringify(message);
                    this.ws.send(serializedMessage);
                    this.metrics.messagesSent++;
    ...
    /**
     * Request data from backend with timeout
     */
    requestData(requestType, payload = {}, timeout = 5000) {
