...
const ConnectionIndicator = ({ connected, size = '12px' }) => {
...
const DiagnosticsPanel = () => {
  ...
  const connectionTestResultRef = useRef(connectionTestResult);
  const [backendInfo, setBackendInfo] = useState(null);

  const connectionInfo = useMemo(checkConnection, [wsConnected]);
  const connectivityInfo = useMemo(checkConnectivity, []);
  ...
  const createTestMessageHandler = useCallback((startTime, setResult) => {
  ...
  const createBackendInfoHandler = useCallback((startTime, setResult) => {
  ...
  const requestBackendInfo = useCallback(async () => {
    const service = useUiStore.getState().wsService;
    if (service) {
      try {
        const startTime = Date.now();
  ...
  const renderConnectionStatus = useCallback(() => {
    return React.createElement('div', { style: { marginBottom: themeUtils.get('SPACING.MD') } },
      React.createElement('div', {
        ...
        React.createElement(ConnectionIndicator, { connected: connectionInfo.connected }),
  ...
  const renderConnectionHistory = useCallback(() => {
    ...
    return React.createElement('div', {
      ...
      ...connectionHistory.slice().reverse().map((entry, index) =>
        React.createElement('div', {
          ...
          React.createElement(ConnectionIndicator, { connected: entry.connected, size: '8px' }),
  ...
  // Memoize diagnostic data sections for better performance
  const connectionDetailsSection = useMemo(() => (
    React.createElement('div', {
      style: {
        padding: themeUtils.get('SPACING.SM'),
        backgroundColor: themeUtils.get('BACKGROUNDS.TERTIARY'),
  ...
  ), [connectionInfo, wsService, wsConnected, wsMetrics]);

  const backendInfoSection = useMemo(() => {
  ...
  const diagnosticButtonsSection = useMemo(() => (
    React.createElement('div', {
      style: {
        display: 'flex',
        gap: themeUtils.get('SPACING.SM'),
  ...
  ), [testConnection, requestBackendInfo]);
