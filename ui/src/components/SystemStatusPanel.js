...
const SystemStatusPanel = () => {
    ...
    if (demoMetrics && Object.keys(demoMetrics).length > 0) {
        // Aggregate metrics from all demo metrics
    ...
    const renderStatusItem = (label, value, unit = '', color = '#000') => 
    ...
    const renderStatusSection = (title, content) =>
        React.createElement('div',
            {
                key: title,
                style: {
    ...
    const systemStatus = React.createElement('div', null,
    ...
    const performanceMetrics = React.createElement('div', null,
    ...
    const memoryUsage = React.createElement('div', null,
    ...
    // Active demo states
    const activeDemoStates = React.createElement('div', null,
        ...Object.entries(demoStates).map(([id, state]) => 
            React.createElement('div', 
                {key: id, style: {marginBottom: '0.5rem', padding: '0.25rem', backgroundColor: '#ffffff', border: '1px solid #dee2e6', borderRadius: '3px'}},
    ...
    const renderStatusItem = (item, index) => {
        if (item.type === 'header') {
            ...
            return renderStatusSection(item.title, item.content);
        }
        return null;
    };
    ...
    return React.createElement(GenericPanel, {
        ...
        emptyMessage: 'System status information will be displayed here once connected to the reasoning engine.'
    });
};
