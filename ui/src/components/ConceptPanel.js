...
const ConceptPanel = () => {
    ...
    const renderControlBar = useCallback(() => createControlBar(React, {
        children: [
    ...
    const renderTimelineView = useCallback(() => {
        ...
        return React.createElement('div', null,
            ...
            React.createElement('div', {style: {display: 'flex', flexDirection: 'column', gap: '1rem'}},
                timelineData.map((item, index) => createTimelineItem(React, {
                    key: item.term,
                    index,
                    children: [
    ...
    const items = useMemo(() => showTimeline ? [
    ...
    const renderItem = useCallback((item, index) => {
        switch (item.type) {
            case 'controls': return item.controlBar;
            case 'timeline': return item.content;
            case 'concept': return renderConcept(item.data, index);
            default: return null;
        }
    }, [renderConcept]);
