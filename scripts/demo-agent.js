...
async function main() {
    ...
    // 1. Setup MCP Server (Simulating an external MCP service)
    ...
    // Register a Calculator tool to the MCP Server
    // Note: We provide 'handler' inside config to satisfy Safety validation,
    // and also as 3rd arg if Server.js expects it there.
    const calcHandler = async (args) => {
        const { operation, a, b } = args;
        console.log(`[MCP Server] Executing calculator: ${operation}(${a}, ${b})`);
        switch (operation) {
            case 'add': return a + b;
    ...
    if (agent.lm) {
        ...
        if (lmProvider && agent.tools && agent.tools.registry) {
