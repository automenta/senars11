...
export class LMConfig {
    ...
    async _configureProvider(providerType, inquirer) {
        ...
        if (providerType === 'ollama') {
            return await inquirer.prompt([
                { type: 'input', name: 'modelName', message: 'Enter Ollama model name:', default: providerInfo.defaultConfig.modelName },
                { type: 'input', name: 'baseURL', message: 'Enter Ollama API URL:', default: providerInfo.defaultConfig.baseURL }
            ]).then(cfg => ({ ...providerInfo.defaultConfig, ...cfg, type: providerType }));
        }

        if (providerType === 'openai' || providerType === 'anthropic') {
            return await inquirer.prompt([
                { type: 'input', name: 'modelName', message: `Enter ${providerInfo.name} model name:`, default: providerInfo.defaultConfig.modelName },
    ...
    async quickSelect() {
        ...
        const modelMap = {
            ...
            'huggingface-smollm': { provider: 'huggingface', modelName: 'HuggingFaceTB/SmolLM-135M-Instruct', device: 'cpu' },
            'huggingface-granite': { provider: 'huggingface', modelName: 'ibm-granite/granite-3.0-2b-instruct', device: 'cpu' },
