export class ProviderUtils {
    static async standardGenerate(provider, prompt, options) {
        if (provider.generateText) return provider.generateText(prompt, options);
        if (provider.invoke) {
    ...
    static async standardStream(provider, prompt, options) {
        if (provider.streamText) return provider.streamText(prompt, options);
        if (provider.stream) return provider.stream(prompt, options);
