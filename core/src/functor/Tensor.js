export class Tensor {
    ...
    _indexToCoords(index, strides) {
        return strides.map(s => { const c = Math.floor(index / s); index %= s; return c; });
    }
    ...
    toArray() { return this._unflatten(this.data, this.shape); }
    toString() { return `Tensor(shape=${this.shape.join('x')}, data=${JSON.stringify(this.toArray())})`; }
