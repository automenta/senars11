export class LossFunctor {
    ...
    _createTensor(data, shape, options = {}) {
        const tensor = new (options.TensorClass || Object.getPrototypeOf(this).constructor)(...[]);
    ...
    _clipTensor(tensor, eps, TensorClass) {
        const clipped = new TensorClass(
            tensor.data.map(x => Math.max(eps, Math.min(1 - eps, x))),
        ...
        this._attachGradient(loss, [predicted, target], () => {
            if (predicted.requiresGrad) {
                ...
                predicted.grad = predicted.grad
                    ? this.backend.add(predicted.grad, scaledGrad)
                    : scaledGrad;
            }
        });
