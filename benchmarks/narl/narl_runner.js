...
class NARLBenchmarkRunner {
    ...
    async runAllLevels() {
        ...
        for (let level = 1; level <= this.totalLevels; level++) {
            ...
            try {
                ...
                this.results[level] = { score: 0, description: `Error: ${error.message}` };
            }
            console.log('');
        }
    ...
    async runLevel1() {
        ...
        // Ask for the derived conclusion
        const result = await brain.ask('(A --> C)?');
    ...
    async runLevel2() {
        ...
        await brain.learn('(bird --> not-flyer). %0.8;0.85%');
        
        // Check if system properly revised the belief
        const result = await brain.ask('(bird --> flyer)?');
    ...
    async runLevel4() {
        ...
        await brain.learn('(rain --> wet-ground).');
        await brain.learn('(wet-ground --> slippery).');
        
        // Ask about causality
        const result = await brain.ask('(rain --> slippery)?');
    ...
    async runLevel10() {
        ...
        const result = await brain.ask('(flying-fish --> swimmer)?'); // Novel concept combination
