...
async function runConsistencyProofDemo() {
    ...
    await brain.learn('(cats --> mammals).');
    await brain.learn('(mammals --> animals).');
    await brain.learn('(cats --> not_animals).');

    console.log('  ✅ Added: (cats --> mammals).');
    console.log('  ✅ Added: (mammals --> animals).');
    console.log('  ⚠️  Added contradictory fact: (cats --> not_animals).');
...
async function runMemoryPersistenceDemo() {
    ...
    const result = await brain.ask('(earth --> celestial_body)?');
