# SeNARS Reasoner Redesign Specification (Revision 2)
...
## 2. High-Level Architecture: A Dataflow Pipeline
...
4.  **Processing:** The `RuleProcessor` consumes the premise pairs. It uses the `RuleOptimizer` to find matching rules efficiently. It processes these rules in a non-blocking fashion:
...
### 3.3. `RuleOptimizer` (Previously `RuleIndexer`)
...
> **Implementation Note:** The guard optimization can be a build-time or startup-time step. The `RuleOptimizer` would analyze the static `Rule` definitions and compile them into an efficient, executable function or data structure that represents the decision tree.
...
*   For each pair, use the `RuleOptimizer` to get a set of candidate rules.
...
*   **Efficiency:** Upgraded the `RuleIndexer` to a `RuleOptimizer` with a mandate to perform advanced **symbolic guard analysis** (deduplication, ordering), moving beyond simple indexing.
